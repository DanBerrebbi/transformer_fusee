...
class Encoder_Decoder(torch.nn.Module):
    ...
    def forward(self, src, sim, pre, tgt, msk_src,msk_sim, msk_pre, msk_tgt):
        ...
        z_pre = self.stacked_encoder_pre(pre, msk_pre)  # [bs,ls,ed]
        ### encoder src #####
        src = self.add_pos_enc(self.src_emb(src))  # [bs,ls,ed]
        z_src = self.stacked_encoder_src(src, msk_src, z_sim, msk_sim, z_pre, msk_pre)  # [bs,ls,ed]
        ### decoder #####
    ...
    def encode_src(self, src, msk_src, z_sim, msk_sim, z_pre, msk_pre):               # je trouve la fonction encode et la fonction decode redondante avec forward
        src = self.add_pos_enc(self.src_emb(src))  # [bs,ls,ed]
        z_src = self.stacked_encoder_src(src, msk_src, z_sim, msk_sim, z_pre, msk_pre)  # [bs,ls,ed]
        return z_src
    ...
    def encode_pre(self, pre, msk_pre):               # je trouve la fonction encode et la fonction decode redondante avec forward
        pre = self.add_pos_enc(self.pre_emb(pre))  # [bs,ls,ed]
        z_pre = self.stacked_encoder_pre(pre, msk_pre)  # [bs,ls,ed]
        return z_pre
...
class Stacked_Encoder_src(torch.nn.Module):
    ...
    def forward(self, src, msk_src, z_sim, msk_sim, z_pre, msk_pre):
        for i, encoderlayer in enumerate(self.encoderlayers):
            src = encoderlayer(src, msk_src, z_sim, msk_sim, z_pre, msk_pre)  # [bs, ls, ed]
...
class Stacked_Encoder_pre(torch.nn.Module):
    ...
    def forward(self, pre, msk_pre):
        for i, encoderlayer in enumerate(self.encoderlayers):
            pre = encoderlayer(pre, msk_pre)  # [bs, ls, ed]
...
class Encoder_src(torch.nn.Module):
    def __init__(self, ff_dim, n_heads, emb_dim, qk_dim, v_dim, dropout):
        ...
        self.multihead_attn_enc_sim = MultiHead_Attn_RELU(n_heads, emb_dim, qk_dim, v_dim, dropout)
    ...
    def forward(self, src, msk_src, z_sim, msk_sim, z_pre, msk_pre):
        ...
        # ATTN over pre words : q are pre words, k, v are src words
        tmp2 = self.multihead_attn_enc_pre(q=tmp1, k=z_pre, v=z_pre, msk=msk_pre)  # [bs, lt, ed] contains dropout
        # ADD
        tmp = tmp2 + tmp
